...
int main(int argc, char **argv)
{
  ...
  objects[0] = (Object){ .m = 60.0, .y = -19.9, .vy = 2.0, .x = 0, .vx = 3};
  objects[1] = (Object){ .m = 100000.0, .y =  1000.0, .vy = 0.0, .x = 0, .vx = 0};
  objects[2] = (Object){ .m = 60.0, .y =  -15, .vy = 0.0, .x = 5, .vx = -4};


  int num_stage = 3; //ステージの数
  Stage stage[num_stage];
  ...
  for (size_t i = 0 ; t <= stop_time ; i++){
    ...
    my_plot_objects(objects, objnum, t, cond, stage, num_stage);
...
void my_plot_objects(Object objs[], const size_t numobj, const double t, const Condition cond, Stage stage[], int num_stage){
    for(int y = -1*cond.height/2 -1; y < cond.height/2 + 1 ; y++){
        for(int x = -1*cond.width/2 - 1 ; x < cond.width/2 + 1 ; x++){
            if(x == -1*cond.width/2 - 1){
               ...
               for(int j = 0; j < stage[0].block_number; j++){
                    if((stage[0].block_position_x[j] <= x) && (stage[0].block_position_x[j] + stage[0].block_width[j] >= x)){
                        if((stage[0].block_position_y[j] <= y) && (stage[0].block_position_y[j] + stage[0].block_height[j] >= y)){
                            printf("\b\e[41m \e[0m");
                        }
                    }
                }
