...
typedef enum res{ EXIT, LINE, UNDO, SAVE, UNKNOWN, ERRNONINT, ERRLACKARGS, NOCOMMAND, RECT, CIRCLE} Result;
...
void draw_circle(Canvas *c, const int x0, const int y0, const int r){
    ...
    for(int y = 0; y < height; y++){
        for(int x = 0; x < width; x++){
            if((((x - x0)*(x - x0) + (y - y0)*(y - y0)) <= r*r + 5) && (((x - x0)*(x - x0) + (y - y0)*(y - y0)) >= r*r - 5)){
                c->canvas[y][x] = pen;
            }
        }
    }
